...
#if defined(NDEBUG)
# error "Apollocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x86d97e089c8067493dfb08c64655b5cb19a2bb85a3701023e1edb7dbe66fec75");
...
{
    // Apollocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Apollocoin: 100k blocks in 5 weeks 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Apollocoin: 1 day
static const int64 nTargetSpacing = 30; // Apollocoin: 30 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Apollocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Apollocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Oscar winner Robin Williams dies at 63";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407984424;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5788880;
        ...
        assert(block.hashMerkleRoot == uint256("0xf42dad9bb2611197463dd95db2bb7239495110036aac33a8ffefda1a2fd45d80"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ApollocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ApollocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ApollocoinMiner(CWallet *pwallet)
{
    printf("ApollocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("apollocoin-miner");
    ...
    try { loop {
        ...
        printf("Running ApollocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ApollocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ApollocoinMiner, pwallet));
}
